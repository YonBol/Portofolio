...
<template>
  <section id="work" class="bg-slate-50 bg-opacity-75 dark:bg-bgDarkSecondary">
    <div class="max-w-custom m-auto flex flex-col gap-8 pt-20 pb-20 w-5/6">
      <div class="flex flex-col gap-3 text-center">
        <h2 class="title">{{ t('work.title') }}<span class="text-primary">.</span></h2>
        <p class="section-subtitle">{{ t('work.subtitle') }}</p>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
        <div
          v-for="project in tm('work.items')"
          :key="project.title"
          class="glow-card bg-bgLightSecondary dark:bg-bgDark rounded-xl overflow-hidden flex flex-col h-full"
        >
          ...
          <div class="p-5 flex flex-1 flex-col gap-5 border-t-2 border-primary">
            ...
            <div class="flex gap-3 w-full justify-start">
              ...
              <a :href="project.demo" target="_blank" rel="noopener noreferrer" aria-label="Demo" class="btn text-sm">{{ t('work.demoBtn') }}
